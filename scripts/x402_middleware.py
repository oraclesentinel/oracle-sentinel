...
def create_payment_requirements() -> dict:
    """Create 402 response - asset as STRING (mint address)"""
    config = get_config()
    ...
    return {
        ...
        "accepts": [{
            ...
            "asset": config["usdc_mint"],  # STRING, not object!
...
def x402_protected(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        payment_header = request.headers.get('X-Payment') or request.headers.get('X-PAYMENT') or request.headers.get('PAYMENT-SIGNATURE')
